...
From the exploratory data analysis (EDA), we can see that the n_hens and n_eggs columns are encoded numerically whereas prod_type and prod_process are categorical. We can also determine that the n_hens column has 9 NA values that must be dealth with, and there are 320 rows in total, a relatively low number. The source column does not provide any useful information for our analysis and so it was not used.
...
We can see that the distributions for both the number of eggs per month and the number of hens per month do not follow any clear trends, which is likely due to the relatively small sample size. We also see that the two distributions are very similar. This was further supported, as the number of eggs and the number of hens had a correlation value of 0.9979883, meaning there is likely a strong relationship between the number of hens and the number of eggs.
...
Finally, we can see that there are more examples of table eggs than hatching eggs and the most examples of the 'all' process type. Whereas the number of examples of 'cage-free (organic)' versus 'cage-free (non-organic)' product types were similar.
...
Additionally, it is important to note that the "cage-free" production process is only applicable to table eggs, not hatching eggs. To ensure a fair and accurate comparison between hatching and table eggs, we filtered the dataset to include only entries where the "prod_process" variable was labeled as "all".
