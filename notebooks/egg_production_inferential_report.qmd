...
From the exploratory data analysis (EDA), we can see that the n_hens and n_eggs columns are encoded numerically whereas prod_type and prod_process are categorical. We can also determine that the n_hens column has 9 NA values that must be dealt with, and there are 320 rows in total, a relatively low number. The source column does not provide any useful information for our analysis and so it was not used.
